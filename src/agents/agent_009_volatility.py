...
Spec: docs/AGENT_009_VOLATILITY.md v2.1
...
def make_volatility_task(agent: Agent, ticker: str) -> Task:
    return Task(
        description=(
            ...
            "    price_52wk_low, market_cap_usd, iv_30d, runway_months, "
            ...
            "    AND event_type IN ('PDUFA_DATE', 'ADCOMM', 'DATA_READOUT_PHASE3', "
            "    'INTERIM_ANALYSIS') ORDER BY event_date ASC.\n"
            "  - Find next expiration date: today + 30 to 45 days (target DTE window).\n"
            "  - REJECT if ANY high-impact catalyst (market_impact_score ≥ 7) falls "
            "    between today and expiration date + 3 days buffer.\n"
            "  - CAUTION if market_impact_score = 5–6 falls in window.\n"
            "  - IV Leakage Warning: if IV_30d > 60% but no catalyst identified, "
            "    proceed at 50% position size.\n\n"
            "STEP 3 — Pipeline Diversity Check:\n"
            "  - DatabaseQueryTool: SELECT COUNT(*) FROM studies "
            "    WHERE ticker = '{ticker}' AND status IN "
            "    ('ACTIVE_RECRUITING', 'ACTIVE_NOT_RECRUITING') AND phase IN "
            "    ('PHASE2', 'PHASE3').\n"
            ...
            f"  ticker={ticker}, status (APPROVED|REJECTED|CAUTION),\n"
            "  calculated_floor, selected_strike, selected_expiry, dte,\n"
            "  premium_usd, annualized_return_pct, iv_source, rejection_reason (if any).\n\n"
            "Return JSON: {\"ticker\": str, \"status\": str, \"calculated_floor\": float, "
            "\"selected_strike\": float|null, \"selected_expiry\": str|null, "
        ...
        expected_output=(
            ...
            "and if APPROVED: selected_strike, selected_expiry (YYYY-MM-DD), dte, "
