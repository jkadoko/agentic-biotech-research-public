...
def make_strategist_task(agent: Agent, ticker: str) -> Task:
    return Task(
        description=(
            ...
            "  - DatabaseQueryTool: SELECT kill_switch FROM agent_profiler_findings "
            "    WHERE ticker = '{ticker}' ORDER BY finding_date DESC LIMIT 1.\n"
            ...
            "    kill_switch IN ('BANKRUPTCY_IMMINENT', 'MANAGEMENT_FRAUD')\n"
            ...
            "  - Profiler: SELECT profiler_score, tam_usd, rnpv_usd, competition_score, "
            "    competitive_advantage, company_type, patent_cliff FROM agent_profiler_findings "
            "    WHERE ticker = '{ticker}' ORDER BY finding_date DESC LIMIT 1.\n"
            "  - Peer Reviewer: SELECT science_score, verdict, competitive_advantage, "
            "    endpoint_switched FROM agent_scientific_audits "
            "    WHERE ticker = '{ticker}' ORDER BY audit_date DESC LIMIT 1.\n"
            "  - Insider: SELECT signal, conviction_score, cluster_detected FROM "
            "    agent_insider_findings WHERE ticker = '{ticker}' ORDER BY analysis_date DESC LIMIT 1.\n"
            "  - Oracle: SELECT event_type, event_date, market_impact_score, confidence "
            ...
            "    WHERE ticker = '{ticker}' ORDER BY analysis_date DESC LIMIT 1.\n"
            "  - Partnership: SELECT partner_name, partner_tier, partnership_type, "
            ...
            "    WHERE ticker = '{ticker}' ORDER BY analysis_date DESC LIMIT 1.\n"
            "  - Financial: SELECT current_price_usd, market_cap_usd, cash_usd, "
            "    cash_per_share, runway_months, iv_30d FROM companies WHERE ticker = '{ticker}'.\n"
            ...
            "  B. Science component (0–25): science_score × 0.25\n"
            ...
            "     sm_norm = sm_conviction_score × 10\n"
            ...
            "     8 pts: DATA_READOUT_PHASE3 within 30–90 days\n"
            "     7 pts: ADCOMM within 30–90 days\n"
            "     5 pts: DATA_READOUT_PHASE2 or CONFERENCE_ORAL within 14–30 days\n"
            ...
            "  - Risk factors: list all concerns (conflicting_signal, endpoint_switched, "
